const StatCard = ({ title, value, subtitle, highlight, success, glow }) => {
  return (
    <div className={`relative p-5 rounded-2xl bg-white/[0.04] border border-white/[0.08] backdrop-blur-md overflow-hidden ${
      highlight ? "ring-1 ring-cyan-400/30" : success ? "ring-1 ring-green-400/30" : ""
    }`}>
      ...
      <div className="relative z-10">
        ...
        <p className={`text-3xl font-bold mt-1 ${highlight ? "text-cyan-400" : success ? "text-green-400" : "text-white"}`}>
          {value}
        </p>
